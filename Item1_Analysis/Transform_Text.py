...
data_dir = 'Item1_Analysis/Description_Data/'
selected_year = '2019'

with open(data_dir+'Item1.json','r') as infile:
    description_data = json.load(infile)

years_available = [year for year in description_data.keys()]

#chose 2019 to maximize observations 
#for year in years_available:
    #print(year,len(description_data[year].keys()))
#slecetd_tickers = description_data[selected_year].keys()
#print(slecetd_tickers)

corpus = description_data[selected_year]
#print(corpus['AAPL'])

vsm = Vector_Space_Model(corpus=description_data[selected_year])
tfidf_df = vsm.tf_idf(norm_length=True)
tfidf_df.to_csv(data_dir+'tfidf_'+str(selected_year)+'.csv')
print(selected_year,'tf-idf data saved')

ticker_list = [ticker for ticker in tfidf_df.keys()]


prices_df = pd.DataFrame()
for tick in ticker_list:
    prices = yf.Ticker(tick).history(period = '5y')['Close'].iloc[::-1]
    prices_df[tick] = prices[::30]

returns = prices_df.pct_change()#must manually clean
returns.to_csv(data_dir+str(selected_year)+'_Returns.csv')
print('Returns Saved')
